...
class Header
  ...
  def self.from_stream(stream)
    ...
    $logger.debug { "Header reported as #{header_size} bytes in length"}
  ...
  def self.from_bytes(bytes)
    ...
    when 11
      $logger.debug { "Parsing 12 byte header" }
      ByteSpec::bytes_to_hash(SPEC_12, bytes)
    when 13
      $logger.debug { "Parsing 14 byte header" }
      ByteSpec::bytes_to_hash(SPEC_14, bytes)
    else
      $logger.debug { "Don't know how to parse unknown byte length header." }
      raise UnknownFormat.new("Cannot read header of size #{bytes.size} bytes")
    end

    $logger.debug { JSON.pretty_generate(data_hash) }
